...
# Running SAVER to confirm the results
```{r}

sce <- readRDS(here("data/sce_neuron.rds"))

# Identify cells with "Glu" in their annotation
glu_cells <- grepl("^Glu", sce$predicted_cell_type)

# Subset the SingleCellExperiment object
sce_glu <- sce[, glu_cells]

cl <- makeCluster(8)
registerDoParallel(cl)

#saver <- saver(sce@assays@data$counts, ncores=8, estimates.only = TRUE)

genes <- c("Gfral","Adcyap1","Glp1r","Gpc3")
genes.ind <- which(rownames(sce_glu@assays@data$counts) %in% genes)

# Generate predictions for those genes and only those

saver.genes.only <- saver(sce_glu@assays@data$counts, pred.genes = genes.ind, 
                                 pred.genes.only = TRUE)


# Run cor.genes() on the two genes
saver.cor.gene <- cor.genes(saver.genes.only)
saver1.cor.cell <- cor.cells(saver.genes.only)

# Assuming 'gene_list' is your list of genes# Replace with your actual gene names

# Filter the SingleCellExperiment object for the specified genes
sce_filtered <- sce[rownames(sce) %in% genes]
# Normalize the filtered data
sce_filtered <- scran::normalize(sce_filtered)
