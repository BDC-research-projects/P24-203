...
joined_seurat <- readRDS(here("data/joined_seurat_integrated.rds"))
all_results <- readRDS(here("data/cell_anno_results.rds"))
...
get_overlap_by_cluster <- function(seurat_obj, gene1, gene2) {
  ...
  for (cluster in cluster_levels) {
    ...
    new_row <- setNames(data.frame(cluster, gfral_pct, adcyap1_pct, overlap_pct, num_cells),
...
saveRDS(overlap_results, here("data/overlap_expr_all_cells.rds"))
```
...
print(ft1)
print(ft2)
ggsave(here("img/expression_gfral_adcyap1_neurons.png"), plot = ft1, width = 10, height = 6, dpi = 300)
ggsave(here("img/Co-expression_gfral_adcyap1_neurons.png"), plot = ft2, width = 20, height = 12, dpi = 600)
```
...
print(ft1)
print(ft2)
ggsave(here("img/expression_dbh_adcyap1_neurons.png"), plot = ft1, width = 10, height = 6, dpi = 300)
ggsave(here("img/Co-expression_dbh_adcyap1_neurons.png"), plot = ft2, width = 20, height = 12, dpi = 600)
...
clusters_of_interest <- c("Glu4","Glu5", "Glu2", "Glu11")
subset_data <- subset_data[subset_data$predicted_cell_type %in% clusters_of_interest, ]

# Convert to long format for ggplot
library(tidyr)
subset_data_long <- pivot_longer(subset_data, cols = c("Dbh", "Adcyap1"), names_to = "Gene", values_to = "Expression")

# Create scatter plot
g <- ggplot(subset_data, aes(x = Dbh, y = Adcyap1, color = predicted_cell_type)) +
  geom_point(alpha = 0.5) +
  facet_wrap(~predicted_cell_type) +
  theme_minimal() +
  labs(title = "Dbh vs. Adcyap1 Expression", 
       x = "Dbh Expression (logcounts)", 
       y = "Adcyap1 Expression (logcounts)")
g

ggsave(here("img/dbh_adcyap1_scatter_plots.png"), g, bg = 'white')
```
...
# Ensure the marker genes are present in your dataset
ap_markers <- intersect(unlist(ap_markers$gene), rownames(neuron_seurat@assays$RNA))
nts_markers <- intersect(unlist(nts_markers$gene), rownames(neuron_seurat@assays$RNA))

region_markers <- list(ap_markers, nts_markers)
names(region_markers) <- c("AP", "NTS")
all.sets <- lapply(names(region_markers), function(x) {
    GeneSet(region_markers[[x]], setName=x)        
})
all.sets <- GeneSetCollection(all.sets)
...
print(all_cells)
ggsave(here("img/AP_NTS_neuron_cells_plot.png"), all_cells)
```
...
print(neuron_ann)
ggsave(here("img/neuron_anno_clusters.png"), plot = neuron_ann, width = 10, height = 6, dpi = 300)
```
...
#saveRDS(results,"neuron_res.rds")
...
gfral_adcyap1_values <- sapply(results_dbh, function(x) {
  val <- x["Dbh", "Adcyap1"]
...
```{r message=FALSE, warning=FALSE}
p_glu4 <- pheatmap(results_dbh[["Glu2"]])
print(p_glu4)
ggsave(here("img/heatmap_glu2_cluster.png"),p_glu4)
```

# Check how many cells from Glu4 overlap with AP
```{r}
# Count total cells in Glu4
total_glu4 <- sum(neuron_seurat$neuron_cell_type == "Glu4")

# Count overlapping cells (cells that are both Glu4 and AP)
overlapping_cells <- sum(neuron_seurat$neuron_cell_type == "Glu4" & neuron_seurat$AP_NTS_Cluster_Annotation == "AP")
